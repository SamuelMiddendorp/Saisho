...
public abstract class SaishoGame_dep {
        ...
        while ( !glfwWindowShouldClose(window) ) {
            ...
            if(inputManager.mouseButtons[0]){
                ...
                float[] vertices = {
                        ...
                        camera.getPosition().x + currentRay.x * 1000, camera.getPosition().y + currentRay.y * 1000, camera.getPosition().z  + currentRay.z * 1000
                };
//                float[] toOrigin = {
...
//                };
                ...
                Vector3f intersection = getIntersectingPlane(camera.getPosition(), currentRay);

                //RawModel line = loader.loadToVao(verticesOrtho);
                RawModel line = loader.loadToVao(vertices);
            ...
            //Vector3f intersection = getIntersectingPlane(new Vector3f(verticescam[0],verticescam[1],verticescam[2]), new Vector3f(verticescam[3], verticescam[4], verticescam[5]));
            Vector3f intersection = getIntersectingPlane(camera.getPosition(), mousePicker.getCurrentRay());
            RawModel lineLook = loader.loadToVao(new float[]{player.getPosition().x, player.getPosition().y, player.getPosition().z, verticescam[0], verticescam[1], verticescam[2]});
            //intersection = new Vector3f(0 ,  1f, -20);
            intersection.y = 0.2f;
            //player.lookAt(intersection);
            //player.setRotation(new Vector3f(1, 1, 0), (float)Math.toRadians(Math.sin(glfwGetTime()) * 360));
